...
def script(jobs_file):
    ...
    """
    #loader job filen ind som indeholder specificationer om det der skal køres
    with open(jobs_file) as f:
        jobs_yaml = f.read()
    jobs = yaml.load(jobs_yaml, Loader=yaml.FullLoader)
    config_state = ConfigState()
    for n, item in enumerate(jobs):
        #Her kan være flere jobs i en yaml fil. Men vi har normalt kun en. Derfor lopper vi kun over et job her.
        #Hvis man havde flere kørte den resten af koden inde i dette loop. 
        config = config_state.update(item['job'])
        #denne funktion kører resten af jobbet 
        process_job(n, config)

#Her køres jobbet, 
def process_job(n, config):
    print('Her bør streamlit åbne')
    ...
    if config['show_plots']:
        st.write('figurene er gemt. Hurra ')
        # plt.show()
...
def nrn_test(n):
    print(n)
    # dataframe_til_test = pd.DataFrame({'A':[1,2,3], 'B':[10,14,45]})
    dataframe_til_test = 4
    
    return dataframe_til_test
