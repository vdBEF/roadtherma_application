...
config['reader'] = st.selectbox('Choose a camera type if "Default" does not work', reader_list,index=3, placeholder="Choose an option",key='reader', on_change=counter_func, help='If the camera type does not work write to Roadtherma@vd.dk, with the type of camera and include the file.' )
...
if run_trimming_checkbox==False and uploaded_file != None and config['reader'] != None and st.session_state.CS != st.session_state.count:# and bool(st.session_state['uploaded_data']!=None)==True:  
  ...
  st.rerun()
    ...
    if coldlines==True:
        ...
        temperatures_trimmed.drop(columns=temperatures_trimmed.columns[list(map(int,(list(map(lambda x: (x)/config['pixel_width']-FP,L1)))))],inplace=True)
        #if config['pixel_width']==0.03:
    ...
    if coldlines==True:
        ...
        if len(L1)>=1:
             ...
             for kkl1 in range(len(L1)):
                 # ax1.axvline(L1[kkl1],color='b',linewidth=1  )
                 if config['pixel_width']==0.03:
                     if len(L1)<3:
                         ax1.axvline(L1[kkl1],color='b',linewidth=1  )
                     elif len(L1)<5:
                         ax1.axvline(L1[kkl1],color='b',linewidth=0.3  )    
                     else:    
                         ax1.axvline(L1[kkl1],color='b',linewidth=0.25  )
                 else:
                     ax1.axvline(L1[kkl1],color='b',linewidth=1  )  
